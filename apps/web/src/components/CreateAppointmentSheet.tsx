  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative w-full h-[calc(100svh-64px)] bg-panel-base rounded-t-[32px] shadow-[0_-8px_32px_rgba(0,0,0,0.18)] flex flex-col animate-in slide-in-from-bottom duration-300 overflow-hidden">
        ...
        {/* Handle (optional, can be removed if header is enough, but usually good for sheets) */}
        <div className="w-full flex justify-center pt-3 pb-1 shrink-0 bg-panel-base">
          <div className="w-12 h-1.5 bg-panel-border rounded-full" />
        </div>

        {/* Header */}
        <div className="flex items-center gap-3 px-6 pb-4 shrink-0 border-b border-panel-border-subtle bg-panel-base">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {clientSearch && (
                    <div className="flex flex-col gap-2 bg-panel-surface border border-panel-border-subtle rounded-xl p-2 mt-2 shadow-sm animate-in fade-in slide-in-from-top-2">
                      ...
                      <button 
                        onClick={() => {
                          setDraft({ ...draft, clientId: "client-1", clientName: "Алина Смирнова" })
                          setClientSearch("")
                        }}
                        className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-panel-base transition-colors border border-transparent hover:border-panel-border-subtle text-left"
                      >
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {["Стрижка", "Окрашивание", "Маникюр"].map(s => (
                  ...
                  >
                    <span className="text-sm font-semibold block mb-1">{s}</span>
              ...
              <div className="flex-1 flex flex-col gap-3">
                ...
                <div className="p-3.5 bg-panel-surface border border-panel-border-subtle rounded-xl flex items-center justify-between">
                  <div className="flex items-center gap-3 w-full justify-between">
                    <button className="w-8 h-8 rounded-full bg-panel-base border border-panel-border flex items-center justify-center hover:border-panel-text-muted transition-colors active:scale-95 text-panel-text font-medium shrink-0">-</button>
                    <div className="flex items-center justify-center gap-1">
                      <input type="number" defaultValue="1" className="w-8 text-center bg-transparent border-b border-panel-border-subtle focus:border-panel-text outline-none text-sm font-semibold text-panel-text" />
                      <span className="text-xs text-panel-text-muted">ч</span>
                      <input type="number" defaultValue="30" className="w-8 text-center bg-transparent border-b border-panel-border-subtle focus:border-panel-text outline-none text-sm font-semibold text-panel-text" />
